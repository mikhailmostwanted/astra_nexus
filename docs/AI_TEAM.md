...
- читает artifact summary, если run уже создавал `artifacts/`;
...
- `/status` и `/runs` показывают `artifacts` и `primary_artifact`, если последний
  completed run уже сохранил итоговые файлы;
...
- `/status` - возвращает состояние активной задачи, а без active job смотрит последний
  terminal run на диске. Для completed run показывает artifact count и primary artifact,
  если они есть.
...
В `TEAM_TELEGRAM_PROVIDER=nodriver` пользователь получает только реально скачанный файл
из ChatGPT Web (`requested_file_download_result.success=true`). Локальный
`requested_output.*` остаётся fallback/debug для fake preview и не считается успешным
NoDriver file-delivery.
...
`astra_nexus.team.artifacts` создаёт пользовательский слой итоговых файлов поверх
существующего workspace. Это не меняет agent pipeline и не заменяет внутренние
`results.json`, `review_protocol.md` или agent results: артефакты просто собирают
понятный финальный пакет после `completed` run.

Модели:

- `TeamArtifact` - один итоговый файл с типом, форматом, путём, размером и флагом primary.
- `TeamArtifactType` - `final_answer`, `executive_summary`, `critic_report`,
  `qa_report`, `review_protocol`, `source_files_summary`, `run_manifest`,
  `artifacts_index`, `requested_output`.
- `TeamArtifactFormat` - `markdown`, `json`, `text`, `docx`, `pdf`.

После успешного run workspace получает:

```text
data/team_runs/<run_id>/artifacts/
  final_answer.md
...
Если пользователь просил файл, дополнительно создаётся requested-file слой:

```text
data/team_runs/<run_id>/
  requested_files/
    downloads/
    <downloaded requested file>
  requested_file_request.json
  requested_file_download_result.json
  artifact_detector_debug.json
```

Для NoDriver happy path `requested_files/<file>` приходит из ChatGPT Web UI. Если
ChatGPT вернул только текст без file card/download button, это ошибка, а не success:
NoDriver делает один retry с прямой инструкцией создать downloadable file и затем
сохраняет debug в `artifact_detector_debug.json`.

Содержание:

- `final_answer.md` - финальный ответ `final_composer`, не обрезается для main chat.
- `executive_summary.md` - коротко: что было на входе, что сделала команда, что получилось,
  какие ограничения/риски остались.
- `critic_report.md` - результат critic и сохранённые review notes критика.
- `qa_report.md` - результат QA, `review_decision`, blocking notes и summary.
- `review_protocol.md` - человекочитаемый review protocol run.
- `source_files_summary.md` - список входных файлов: filename, type/extension, size,
  extraction status, `extracted_chars`, `prompt_chars`, `truncated`, extraction error.
- `run_manifest.json` - machine-readable manifest run + artifacts.
- `index.md` - человекочитаемый индекс: run id, status, задача, входные файлы,
  список артефактов, пути и timestamp.
...
Failed/cancelled run не создаёт ложный `final_answer.md`. Старые workspace без этих полей
остаются читаемыми: registry считает artifact count равным `0`.

Telegram:

- main chat получает финальный ответ как раньше;
- для обычного text answer flow internal artifacts не отправляются по умолчанию;
- `final_answer.md` и `index.md` уходят пользователю только при явном
  `TEAM_TELEGRAM_SEND_INTERNAL_ARTIFACTS=true`;
- если был requested-file flow, main chat получает только скачанный requested file;
- если файл не скачался, main chat получает короткую ошибку, а технические причины и
  пути debug JSON уходят в log chat.

Preview без Telegram API и без реального provider:

```bash
astra-nexus-team-artifacts-preview "собери итог"
astra-nexus-team-artifacts-preview --file docs/AI_TEAM.md "проверь файл"
astra-nexus-team-requested-file-preview "сделай краткий план и пришли docx"
```

CLI запускает `FakeTeamProvider`, создаёт workspace и печатает `run_id`, `status`,
`artifacts_dir` и список созданных artifact files.
...
Когда final composer получает `output_requested_as_file=true`, prompt явно требует
создать настоящий downloadable file в ChatGPT Web. Контекст передаёт в NoDriver:

- `output_requested_as_file`;
- `requested_output_format`;
- `workspace_path`;
- `run_id` / `agent_task_id`.

`ChatGPTClient` после idle-состояния ищет artifact candidates в текущем assistant turn,
ставит Chrome download path в `requested_files/downloads/`, кликает найденную кнопку
или ссылку, ждёт исчезновения `.crdownload`/`.tmp`/partial файлов, проверяет размер и
расширение, затем переносит результат в `requested_files/`.
...
финальный результат, количество attachments, artifact summary и краткий список agent
task/result summary.
...
`artifacts/` содержит итоговый пользовательский пакет Output Artifacts v1. Главный файл -
`final_answer.md`, индекс - `index.md`, machine-readable manifest - `run_manifest.json`.
