...
- `TeamArtifactType` - `final_answer`, `executive_summary`, `critic_report`,
  ...
  `artifacts_index`.
- `TeamArtifactFormat` - `markdown`, `json`, `text`.
...
- затем bridge отправляет короткое сообщение `Файлы результата сохранены: ...`;
- если bot API поддерживает documents, bridge отправляет `final_answer.md` и `index.md`;
- log chat остаётся техническим и не получает полный текст артефактов.
