...
NODRIVER_START_TIMEOUT_SECONDS=30
...
Проверить запуск Chrome и подключение NoDriver без открытия ChatGPT:
...
- версию Python;
- путь до Chrome или Chromium;
- абсолютный `user_data_dir`;
- существует ли папка профиля;
- `headless`, `no_sandbox`, `start_timeout`;
- результат открытия `about:blank`.

Если Chrome установлен нестандартно:

```env
NODRIVER_BROWSER_EXECUTABLE_PATH=/absolute/path/to/chrome
```

Если NoDriver не успевает подключиться к remote debugging port, можно увеличить:

```env
NODRIVER_START_TIMEOUT_SECONDS=60
```

`NODRIVER_NO_SANDBOX=true` стоит использовать только когда среда запуска требует
отключения sandbox, например в некоторых контейнерах.
...
- `browser_connect_failed` - Chrome стартовал, но NoDriver не подключился.
